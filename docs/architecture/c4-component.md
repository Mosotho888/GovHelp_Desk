...
This zooms into the **API** container from [`c4-container.md`](c4-container.md) to show its internal module structure.
GovHelpDesk is organised as a modular monolith: each business domain is a self-contained Java package under
`za.gov.helpdesk.<domain>` with its own
