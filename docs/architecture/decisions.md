...
Significant, hard-to-reverse architectural decisions are recorded as individual
Architecture Decision Records (ADRs) under [`docs/adr/`](../adr/), using the lightweight
Michael Nygard format. This page is a chronological index with a one-line summary of each.
...
This project treats architectural decisions as artifacts worth recording, not just code worth
writing — mainly because the reasoning behind a decision (what was rejected, and why) tends to
get lost once the code just "is what it is." For a portfolio project, ADRs also give a
reviewer a fast way to see engineering judgment, not just engineering output.
