...
| [0008](../adr/0008-knowledge-base.md)                 | Knowledge base with Postgres full-text search and ticket-linked usage tracking             | Accepted | Generated `tsvector`/GIN full-text search over `LIKE` matching or external search infra; `TicketKnowledgeArticle` join mirrors `TicketAsset`; per-user upserted feedback; citizens restricted to `PUBLISHED` content        |
| [0009](../adr/0009-reporting-schema.md)               | Reporting schema built on materialized views (Phase 1)                                     | Accepted | Six Postgres materialized views refreshed every 15 minutes via `REFRESH MATERIALIZED VIEW CONCURRENTLY`, read through typed `NamedParameterJdbcTemplate` row mappers, ahead of a planned event-driven fact table in Phase 2 |
