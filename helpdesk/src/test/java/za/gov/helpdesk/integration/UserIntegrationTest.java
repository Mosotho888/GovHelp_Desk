...
public class UserIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        User john = userRepository.save(User.builder().name("John Public").email("john@citizen.za")
    ...
    void createUser_adminToken_returns201() throws Exception {
        mvc.perform(post("/v1/users").header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("name", "New Citizen", "email", "newcitizen@gov.za",
                        "password", "Citizen@1234", "role", "USER"))))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.email").value("newcitizen@gov.za"))
                .andExpect(jsonPath("$.role").value("USER")).andExpect(jsonPath("$.active").value(true));
    }
    ...
    void createUser_userToken_returns403() throws Exception {
        mvc.perform(
                post("/v1/users").header("Authorization", "Bearer " + userToken).contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(
                                Map.of("name", "Hacker", "email", "hacker@evil.com", "password", "Hack@1234"))))
    ...
    void createUser_duplicateEmail_returns409() throws Exception {
        mvc.perform(post("/v1/users").header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(
                        Map.of("name", "Duplicate", "email", "john@citizen.za", "password", "Pass@1234"))))
    ...
    void createUser_weakPassword_returns400() throws Exception {
        mvc.perform(post("/v1/users").header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("name", "Test", "email", "test@gov.za", "password", "weak"))))
    ...
    void listUsers_adminToken_returns200() throws Exception {
        mvc.perform(get("/v1/users").header("Authorization", "Bearer " + adminToken)).andExpect(status().isOk())
    ...
    void listUsers_userToken_returns403() throws Exception {
        mvc.perform(get("/v1/users").header("Authorization", "Bearer " + userToken)).andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("GET /users/me returns current user profile")
    void getMyProfile_authenticated_returnsOwnProfile() throws Exception {
        mvc.perform(get("/v1/users/me").header("Authorization", "Bearer " + userToken)).andExpect(status().isOk())
    ...
    void getUserById_adminFetchesOtherUser_returns200() throws Exception {
        mvc.perform(get("/v1/users/" + johnId).header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk()).andExpect(jsonPath("$.email").value("john@citizen.za"));
    }
    ...
    void getUserById_idor_returns403() throws Exception {
        ...
        userRepository.save(User.builder().name("Another Citizen").email("another@citizen.za")
                .passwordHash(passwordEncoder.encode("AnotherPass1!")).role(User.Role.USER).active(true)
                .loginAttempts(0).timezone("Africa/Johannesburg").build());

        String anotherToken = login("another@citizen.za", "AnotherPass1!");
    ...
    void updateUser_ownProfile_returns200() throws Exception {
        mvc.perform(put("/v1/users/" + johnId).header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("name", "John Updated", "timezone", "Africa/Johannesburg"))))
                .andExpect(status().isOk()).andExpect(jsonPath("$.name").value("John Updated"));
    }
    ...
    void deactivateUser_adminToken_returns204AndPreventsLogin() throws Exception {
        String body = mvc
                .perform(post("/v1/users").header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(Map.of("name",
                                "To Deactivate", "email", "deactivate@gov.za", "password", "Temp@1234"))))
                .andReturn().getResponse().getContentAsString();
        long userId = mapper.readTree(body).get("id").asLong();

        mvc.perform(delete("/v1/users/" + userId).header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isNoContent());

        // Deactivated users cannot log in
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "deactivate@gov.za", "password", "Temp@1234"))))
                .andExpect(status().isForbidden());
    }
    ...
    void reactivateUser_adminToken_allowsLoginAgain() throws Exception {
        ...
        String body = mvc
                .perform(post("/v1/users").header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(
                                Map.of("name", "Reactivatable", "email", "react@gov.za", "password", "React@1234"))))
                .andReturn().getResponse().getContentAsString();
        long userId = mapper.readTree(body).get("id").asLong();
        ...
        // Reactivate
        mvc.perform(post("/v1/users/" + userId + "/reactivate").header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isNoContent());

        // Should be able to login now
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "react@gov.za", "password", "React@1234"))))
    ...
    void changeRole_adminToken_returns200() throws Exception {
        mvc.perform(patch("/v1/users/" + johnId + "/role").header("Authorization", "Bearer " + adminToken)
                .param("role", "AGENT").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
    ...
    void changeRole_nonAdmin_returns403() throws Exception {
        mvc.perform(patch("/v1/users/" + johnId + "/role").header("Authorization", "Bearer " + userToken)
                .param("role", "ADMIN").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isForbidden());
    }
    ...
    void changeOwnPassword_validCurrentPassword_returns204() throws Exception {
        mvc.perform(patch("/v1/users/me/password").header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper
                        .writeValueAsString(Map.of("currentPassword", "UserPass1!", "newPassword", "NewUserPass1!"))))
                .andExpect(status().isNoContent());

        // Old password no longer works
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "john@citizen.za", "password", "UserPass1!"))))
                .andExpect(status().isUnauthorized());

        // New password works
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "john@citizen.za", "password", "NewUserPass1!"))))
    ...
    void changeOwnPassword_wrongCurrentPassword_returns400() throws Exception {
        mvc.perform(patch("/v1/users/me/password").header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper
                        .writeValueAsString(Map.of("currentPassword", "WrongPass1!", "newPassword", "NewUserPass1!"))))
                .andExpect(status().isUnauthorized());
    }
