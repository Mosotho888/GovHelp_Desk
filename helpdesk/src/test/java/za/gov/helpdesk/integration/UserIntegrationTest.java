...
public class UserIntegrationTest extends BaseIntegrationTest {
    ...
    void getUserById_adminFetchesOtherUser_returns200() throws Exception {
        mvc.perform(get("/v1/users/" + johnId)
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.email").value("john@citizen.za"));
    }
    ...
    void getUserById_idor_returns403() throws Exception {
        ...
        userRepository.save(User.builder()
                .name("Another Citizen").email("another@citizen.za")
    ...
    void updateUser_ownProfile_returns200() throws Exception {
        mvc.perform(put("/v1/users/" + johnId)
                        .header("Authorization", "Bearer " + userToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "name", "John Updated",
    ...
    void reactivateUser_adminToken_allowsLoginAgain() throws Exception {
        ...
        String body = mvc.perform(post("/v1/users")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "name", "Reactivatable", "email", "react@gov.za", "password", "React@1234"
                        ))))
                .andReturn().getResponse().getContentAsString();
        long userId = mapper.readTree(body).get("id").asLong();
        ...
        // Reactivate
        mvc.perform(post("/v1/users/" + userId + "/reactivate")
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isNoContent());
        ...
        mvc.perform(post("/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "email", "react@gov.za", "password", "React@1234"
                        ))))
                .andExpect(status().isOk())
    ...
    void changeRole_adminToken_returns200() throws Exception {
        mvc.perform(patch("/v1/users/" + johnId + "/role")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("role", "AGENT")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void changeOwnPassword_validCurrentPassword_returns204() throws Exception {
        mvc.perform(patch("/v1/users/me/password")
                        .header("Authorization", "Bearer " + userToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
        ...
        mvc.perform(post("/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "email", "john@citizen.za", "password", "UserPass1!"
                        ))))
        ...
        mvc.perform(post("/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "email", "john@citizen.za", "password", "NewUserPass1!"
                        ))))
                .andExpect(status().isOk());
    }
