...
public class CommentIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        slaPolicyRepository.save(SlaPolicy.builder().priority(Ticket.Priority.MEDIUM).resolutionMinutes(480) // Example threshold metric
                .responseMinutes(480).build());
        User citizen = userRepository.save(User.builder().name("John Public").email("john@citizen.za")
        ...
        agent = agentRepository.save(Agent.builder().user(agentUser).availability(Agent.Availability.ONLINE).build());
    ...
    void addComment_validRequest_returns201() throws Exception {
        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + userToken)
    ...
    void addComment_noToken_returns401() throws Exception {
        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").contentType(MediaType.APPLICATION_JSON)
    ...
    void addComment_blankBody_returns400() throws Exception {
        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(Map.of("body", "   "))))
    ...
    void addComment_ticketNotFound_returns404() throws Exception {
        mvc.perform(post("/v1/tickets/99999/comments").header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void addComment_agentInternalNote_hiddenFromUser() throws Exception {
        ...
        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper
                        .writeValueAsString(Map.of("body", "Escalating internally — do not share", "internal", true))))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.internal").value(true));

        // Also post a public comment so user gets at least one visible comment
        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "We are looking into this", "internal", false))))
                .andExpect(status().isCreated());

        // USER listing comments must NOT include the internal note
        mvc.perform(get("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + userToken))
                .andExpect(status().isOk()).andExpect(jsonPath("$.content[*].internal", everyItem(is(false))))
                .andExpect(jsonPath("$.content[*].body", not(hasItem("Escalating internally — do not share"))));

        // Agent listing can see the internal note
        mvc.perform(get("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + agentToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[*].body", hasItem("Escalating internally — do not share")));
    }
    ...
    void getComments_multiplePosts_returnsPaginatedAscending() throws Exception {
        ...
        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "First comment")))).andExpect(status().isCreated());

        mvc.perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "Agent response")))).andExpect(status().isCreated());
    ...
    void addReply_validRequest_returns201() throws Exception {
        ...
        String commentBody = mvc
                .perform(post("/v1/tickets/" + ticketId + "/comments").header("Authorization", "Bearer " + userToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(post("/v1/comments/" + commentId + "/replies").header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "We are on it")))).andExpect(status().isCreated())
    ...
    void getReplies_hasReplies_returnsAll() throws Exception {
        ...
        mvc.perform(post("/v1/comments/" + commentId + "/replies").header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "Reply one")))).andExpect(status().isCreated());

        mvc.perform(post("/v1/comments/" + commentId + "/replies").header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "Reply two")))).andExpect(status().isCreated());

        mvc.perform(get("/v1/comments/" + commentId + "/replies").header("Authorization", "Bearer " + userToken))
    ...
    void updateComment_byAuthorWithinWindow_returns200() throws Exception {
        ...
        mvc.perform(put("/v1/comments/" + commentId).header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "Corrected text")))).andExpect(status().isOk())
    ...
    void updateComment_byOtherUser_returns403() throws Exception {
        ...
        mvc.perform(put("/v1/comments/" + commentId).header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("body", "Agent tampers")))).andExpect(status().isForbidden());
    }
    ...
    void deleteComment_byAuthorWithinWindow_returns204() throws Exception {
        ...
        mvc.perform(delete("/v1/comments/" + commentId).header("Authorization", "Bearer " + userToken))
    ...
    private long createTicket(String token, String subject, String description) throws Exception {
        String body = mvc
                .perform(post("/v1/tickets").header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of("subject", subject, "description", description))))
                .andReturn().getResponse().getContentAsString();
