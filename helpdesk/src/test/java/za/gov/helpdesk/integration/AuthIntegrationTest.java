...
public class AuthIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        userRepository.save(User.builder().name("Test Agent").email("agent@gov.za")
                .passwordHash(passwordEncoder.encode("ValidPass1!")).role(User.Role.AGENT).active(true).loginAttempts(0)
    ...
    void login_validCredentials_returns200WithTokens() throws Exception {
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "agent@gov.za", "password", "ValidPass1!"))))
                .andExpect(status().isOk()).andExpect(jsonPath("$.accessToken").isNotEmpty())
                .andExpect(jsonPath("$.refreshToken").isNotEmpty()).andExpect(jsonPath("$.tokenType").value("Bearer"))
    ...
    void login_wrongPassword_returns401() throws Exception {
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "agent@gov.za", "password", "WrongPassword1!"))))
    ...
    void login_unknownEmail_returns401() throws Exception {
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "nobody@gov.za", "password", "ValidPass1!"))))
    ...
    void login_missingEmail_returns400() throws Exception {
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("password", "ValidPass1!"))))
    ...
    void login_invalidEmailFormat_returns400() throws Exception {
        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "not-an-email", "password", "ValidPass1!"))))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.details[0].field").value("email"));
    }

    @Test
    @DisplayName("POST /auth/login returns 403 for inactive account")
    void login_inactiveAccount_returns403() throws Exception {
        userRepository.save(User.builder().name("Inactive User").email("inactive@gov.za")
                .passwordHash(passwordEncoder.encode("ValidPass1!")).role(User.Role.USER).active(false).loginAttempts(0)
                .timezone("Africa/Johannesburg").build());

        mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "inactive@gov.za", "password", "ValidPass1!"))))
    ...
    void login_fiveFailedAttempts_locksAccount() throws Exception {
        for (int i = 0; i < 5; i++) {
            mvc.perform(post("/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                    .content(mapper.writeValueAsString(Map.of("email", "agent@gov.za", "password", "WrongPass1!"))));
        }
    ...
    void refresh_validRefreshToken_returnsNewAccessToken() throws Exception {
        ...
        mvc.perform(post("/v1/auth/refresh").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("refreshToken", refreshToken)))).andExpect(status().isOk())
    ...
    void logout_revokesRefreshToken() throws Exception {
        ...
        mvc.perform(post("/v1/auth/logout").header("Authorization", "Bearer " + accessToken)
    ...
    void passwordResetRequest_alwaysReturns200() throws Exception {
        // Known email
        mvc.perform(post("/v1/auth/password-reset/request").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "agent@gov.za")))).andExpect(status().isOk());

        // Unknown email - must also return 200 to prevent enumeration
        mvc.perform(post("/v1/auth/password-reset/request").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("email", "ghost@gov.za")))).andExpect(status().isOk());
    }
}
