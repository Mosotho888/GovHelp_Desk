...
public class AgentIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        String body = mvc.perform(post("/v1/agents")
    ...
    void createAgent_adminToken_returns201() throws Exception {
        User newAgentUser = userRepository.save(User.builder()
                .name("Bob Agent").email("bob@gov.za")
                .passwordHash(passwordEncoder.encode("AgentPass1!"))
                .role(User.Role.AGENT).active(true).loginAttempts(0)
                .timezone("Africa/Johannesburg").build());

        mvc.perform(post("/v1/agents")
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "userId", newAgentUser.getId(),
                                "department", "HR",
    ...
    void getAgentById_existingId_returns200() throws Exception {
        mvc.perform(get("/v1/agents/" + agentId)
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk())
    ...
    void getAgentById_notFound_returns404() throws Exception {
        mvc.perform(get("/v1/agents/99999")
                        .header("Authorization", "Bearer " + adminToken))
    ...
    void updateAgent_adminToken_returns200() throws Exception {
        mvc.perform(patch("/v1/agents/" + agentId)
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(Map.of(
                                "department", "Finance",
    ...
    void getAgentStats_adminToken_returns200() throws Exception {
        mvc.perform(get("/v1/agents/" + agentId + "/stats")
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk())
