...
public class AgentIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        String body = mvc
                .perform(post("/v1/agents").header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(
                                Map.of("userId", agentUserId, "department", "IT Support", "availability", "ONLINE"))))
    ...
    void createAgent_adminToken_returns201() throws Exception {
        User newAgentUser = userRepository.save(
                User.builder().name("Bob Agent").email("bob@gov.za").passwordHash(passwordEncoder.encode("AgentPass1!"))
                        .role(User.Role.AGENT).active(true).loginAttempts(0).timezone("Africa/Johannesburg").build());

        mvc.perform(post("/v1/agents").header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(
                        Map.of("userId", newAgentUser.getId(), "department", "HR", "availability", "OFFLINE"))))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.id").isNumber())
                .andExpect(jsonPath("$.department").value("HR")).andExpect(jsonPath("$.availability").value("OFFLINE"))
    ...
    void createAgent_nonAdmin_returns403() throws Exception {
        mvc.perform(post("/v1/agents").header("Authorization", "Bearer " + userToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("userId", agentUserId)))).andExpect(status().isForbidden());
    }
    ...
    void createAgent_duplicateUser_returns409() throws Exception {
        ...
        mvc.perform(post("/v1/agents").header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("userId", agentUserId)))).andExpect(status().isConflict());
    }
    ...
    void getAllAgents_agentOrAdmin_returns200() throws Exception {
        mvc.perform(get("/v1/agents").header("Authorization", "Bearer " + agentToken)).andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(1)))
                .andExpect(jsonPath("$.content[0].user.email").value("jane@gov.za"));

        mvc.perform(get("/v1/agents").header("Authorization", "Bearer " + adminToken)).andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(1)));
    }
    ...
    void getAllAgents_userRole_returns403() throws Exception {
        mvc.perform(get("/v1/agents").header("Authorization", "Bearer " + userToken)).andExpect(status().isForbidden());
    }
    ...
    void getAgentById_existingId_returns200() throws Exception {
        mvc.perform(get("/v1/agents/" + agentId).header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk()).andExpect(jsonPath("$.id").value(agentId))
    ...
    void updateAgent_adminToken_returns200() throws Exception {
        mvc.perform(patch("/v1/agents/" + agentId).header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("department", "Finance", "availability", "BUSY"))))
                .andExpect(status().isOk()).andExpect(jsonPath("$.department").value("Finance"))
    ...
    void updateAgent_ownAgent_returns200() throws Exception {
        mvc.perform(patch("/v1/agents/" + agentId).header("Authorization", "Bearer " + agentToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(Map.of("availability", "AWAY")))).andExpect(status().isOk())
    ...
    void getAgentStats_adminToken_returns200() throws Exception {
        mvc.perform(get("/v1/agents/" + agentId + "/stats").header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk()).andExpect(jsonPath("$.openCount").isNumber())
    ...
    void getAgentStats_nonAdmin_returns403() throws Exception {
        mvc.perform(get("/v1/agents/" + agentId + "/stats").header("Authorization", "Bearer " + agentToken))
