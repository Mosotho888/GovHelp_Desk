...
public class TicketIntegrationTest extends BaseIntegrationTest {
                                ...
                                .content(
                                        mapper.writeValueAsString(
                                                Map.of(
                                                        ...
                                                        "System is down",
                                                        "description",
                                                        "Cannot access the portal since 09:00",
                                                        "priority",
                                                        "HIGH"))))
    ...
    void createTicket_missingSubject_returns400() throws Exception {
        mvc.perform(
                        post("/v1/tickets")
                                ...
                                .content(
                                        mapper.writeValueAsString(
                                                Map.of("description", "No subject provided"))))
    ...
    void getTickets_userRole_seesOnlyOwnTickets() throws Exception {
        ...
        mvc.perform(get("/v1/tickets").header("Authorization", "Bearer " + userToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content[0].requester.email").value("john@citizen.za"));

        mvc.perform(get("/v1/tickets").header("Authorization", "Bearer " + agentToken))
                .andExpect(status().isOk())
    ...
    void getTicketById_idor_returns403() throws Exception {
        ...
        final String johnTicket =
                mvc.perform(
                        ...
                        .getResponse()
                        .getContentAsString();
        final long johnTicketId = mapper.readTree(johnTicket).get("id").asLong();
        ...
        userRepository.save(
                User.builder()
                        ...
                        .timezone("Africa/Johannesburg")
                        .build());
        final String eveToken = login("eve@citizen.za", "EvePass1!");

        mvc.perform(
                        get("/v1/tickets/" + johnTicketId)
    ...
    @Test
    @DisplayName(
            "Full lifecycle: OPEN -> IN_PROGRESS -> RESOLVED -> CLOSED — SLA timestamps recorded")
    void ticketLifecycle_fullFlow_succeeds() throws Exception {
        final String createBody =
                mvc.perform(
                        ...
                        .getResponse()
                        .getContentAsString();
        final long ticketId = mapper.readTree(createBody).get("id").asLong();

        // SLA deadlines are set immediately
        mvc.perform(
                        get("/v1/tickets/" + ticketId + "/sla")
