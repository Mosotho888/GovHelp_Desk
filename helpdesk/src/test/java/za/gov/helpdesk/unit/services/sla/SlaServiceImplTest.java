...
class SlaServiceImplTest {
    ...
    void initializeSla_usesPolicyDeadlines() {
        ...
        given(calculator.addBusinessMinutes(any(LocalDateTime.class), eq(240L))).willReturn(responseDue);
        given(calculator.addBusinessMinutes(any(LocalDateTime.class), eq(480L))).willReturn(resolutionDue);
        given(ticketSlaRepository.save(any(TicketSla.class))).willAnswer(i -> i.getArgument(0));

        TicketSla result = slaService.initializeSla(ticket);
    ...
    void getSlaStatus_onTrack_returnsOnTrack() {
        ...
        TicketSlaResponse response = slaService.getSlaStatus(100L);
