...
class TicketEmailNotificationPublisherTest {
    ...
    void setUp() {
        ...
        requester = User.builder().id(1L).name("John Public").email("john@citizen.za").role(User.Role.USER).build();
        agentUser = User.builder().id(2L).name("Jane Agent").email("jane@gov.za").role(User.Role.AGENT).build();
        ...
        ticket = Ticket.builder().id(100L).subject("Login broken").description("Cannot access dashboard")
                .status(Ticket.Status.IN_PROGRESS).priority(Ticket.Priority.HIGH).requester(requester).assignee(agent)
                .build();
    }
    ...
    void publish_buildsExpectedMessage() {
        ArgumentCaptor<TicketEmailNotificationMessage> captor = ArgumentCaptor
                .forClass(TicketEmailNotificationMessage.class);

        publisher.publish(ticket, requester, agentUser, AuditLog.AuditAction.STATUS_CHANGED, "Working on it");

        then(outboxWriter).should(times(1)).write(eq(OutboxEvent.EventType.TICKET_EMAIL.name()),
                eq(AuditLog.EntityType.TICKET.name()), eq(100L), captor.capture());
    ...
    void publish_outboxFailure_doesNotThrow() {
        doThrow(new IllegalStateException("Failed to write outbox event")).when(outboxWriter).write(
                eq(OutboxEvent.EventType.TICKET_EMAIL.name()), eq(AuditLog.EntityType.TICKET.name()), eq(100L),
        ...
        then(outboxWriter).should(times(1)).write(eq(OutboxEvent.EventType.TICKET_EMAIL.name()),
                eq(AuditLog.EntityType.TICKET.name()), eq(100L), any(TicketEmailNotificationMessage.class));
    }
    ...
    void publish_triggerMappedCorrectly_forEachRelevantAction() {
        for (AuditLog.AuditAction action : new AuditLog.AuditAction[]{AuditLog.AuditAction.TICKET_CREATED,
