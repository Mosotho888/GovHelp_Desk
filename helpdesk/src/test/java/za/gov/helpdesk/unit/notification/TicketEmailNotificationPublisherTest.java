...
class TicketEmailNotificationPublisherTest {
    ...
    void publish_buildsExpectedMessage() {
        final ArgumentCaptor<TicketEmailNotificationMessage> captor =
                ArgumentCaptor.forClass(TicketEmailNotificationMessage.class);

        publisher.publish(
                ticket, requester, agentUser, AuditLog.AuditAction.STATUS_CHANGED, "Working on it");
    ...
    void publish_triggerMappedCorrectly_forEachRelevantAction() {
        for (final AuditLog.AuditAction action :
                new AuditLog.AuditAction[] {
