...
class TicketEmailNotificationConsumerTest {
    ...
    void handle_routesSupportedTriggersAndAcks() throws Exception {
        ...
        then(ticketEmailService).should(times(1)).sendTicketCreated(org.mockito.ArgumentMatchers.any());
        then(ticketEmailService).should(times(1)).sendTicketAssigned(org.mockito.ArgumentMatchers.any());
        then(ticketEmailService).should(times(1)).sendStatusChanged(org.mockito.ArgumentMatchers.any());
        then(ticketEmailService).should(times(1)).sendCommentAdded(org.mockito.ArgumentMatchers.any());
        then(ticketEmailService).should(times(1)).sendTicketClosed(org.mockito.ArgumentMatchers.any());
    }
    ...
    void handle_unsupportedTrigger_acksWithoutRouting() throws Exception {
        TicketEmailNotificationMessage message = message(AuditLog.AuditAction.PRIORITY_CHANGED);
