...
public class CommentServiceImplTest {
    ...
    void addComment_valid_savesAndPublishesAudit() {
        ...
        then(auditPublisher).should(times(1)).publishAudit(
                eq(AuditLog.EntityType.COMMENT), eq(comment.getId()),
                eq(agentUser), eq(AuditLog.AuditAction.COMMENT_ADDED),
                isNull(), any(), any()
        );
    }
    ...
    void addComment_internalByAgent_savesAndPublishesInternalNoteAudit() {
        Comment internalNote = Comment.builder().id(101L).ticket(ticket).author(agentUser)
                .body("Internal: checking database").internal(true)
                .type(Comment.CommentType.NOTE).createdAt(LocalDateTime.now()).build();
        ...
        then(auditPublisher).should(times(1)).publishAudit(
                eq(AuditLog.EntityType.COMMENT), eq(internalNote.getId()),
                eq(agentUser), eq(AuditLog.AuditAction.INTERNAL_NOTE_ADDED),
                isNull(), any(), any()
        );
    }
    ...
    void addReply_valid_linksToParent() {
        ...
        verify(commentRepository, times(1)).save(argThat(saved ->
                saved.getParent().equals(comment)
                        ...
                        && saved.getBody().equals("Thanks for the update!")
        ));
        then(auditPublisher).should(times(1)).publishAudit(
                eq(AuditLog.EntityType.COMMENT), eq(replyComment.getId()),
                eq(endUser), eq(AuditLog.AuditAction.COMMENT_ADDED),
                isNull(), any(), any()
        );
    }
    ...
    void updateComment_valid_updatesAndPublishesAudit() {
        ...
        then(auditPublisher).should(times(1)).publishAudit(
                eq(AuditLog.EntityType.COMMENT), eq(comment.getId()),
                eq(agentUser), eq(AuditLog.AuditAction.COMMENT_EDITED),
                any(), isNull(), any()
        );
    }
    ...
    void deleteComment_valid_publishesAuditAndDeletes() {
        ...
        then(auditPublisher).should(times(1)).publishAudit(
                eq(AuditLog.EntityType.COMMENT), eq(comment.getId()),
                eq(agentUser), eq(AuditLog.AuditAction.COMMENT_DELETED),
                any(), isNull(), any()
        );
