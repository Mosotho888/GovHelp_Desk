...
public class CommentServiceImplTest {
    ...
    @Mock private CommentQueryHelper commentQuery;
    @Mock private AuditEventPublisher auditPublisher;
    @Mock private CommentAccessPolicy accessPolicy;
    ...
    void addComment_valid_savesAndPublishesAudit() {
        final CreateCommentRequest req = new CreateCommentRequest();
        req.setBody("Investigating the issue.");
        req.setInternal(false);

        given(ticketQuery.findOrThrow(10L, agentUser)).willReturn(ticket);
        given(commentRepository.save(any(Comment.class))).willReturn(comment);
        given(commentMapper.toCommentResponse(comment)).willReturn(responseFor(comment));

        final CommentResponse response = commentService.addComment(10L, req, agentUser);
    ...
    void addComment_internalByAgent_savesAndPublishesInternalNoteAudit() {
        ...
        final Comment internalNote =
                Comment.builder()
                        .id(101L)
                        .ticket(ticket)
                        .author(agentUser)
                        .body("Internal: checking database")
                        .internal(true)
                        .type(Comment.CommentType.NOTE)
                        .createdAt(LocalDateTime.now())
                        .build();
        ...
        then(auditPublisher)
                ...
                .publishAudit(
                        eq(AuditLog.EntityType.COMMENT),
                        eq(internalNote.getId()),
                        eq(agentUser),
                        eq(AuditLog.AuditAction.INTERNAL_NOTE_ADDED),
                        isNull(),
                        any(),
                        any());
    }
    ...
    void addComment_internalByUser_throwsAccessDenied() {
        ...
        given(ticketQuery.findOrThrow(10L, endUser)).willReturn(ticket);
    ...
    void addReply_valid_linksToParent() {
        ...
        given(commentQuery.findOrThrow(100L, endUser)).willReturn(comment);
    ...
    void addReply_unknownParent_throwsNotFound() {
        ...
        given(commentQuery.findOrThrow(999L, agentUser))
    ...
    void updateComment_valid_updatesAndPublishesAudit() {
        final UpdateCommentRequest req = new UpdateCommentRequest();
        req.setBody("Corrected body");

        given(commentQuery.findOrThrow(100L, agentUser)).willReturn(comment);
        given(commentRepository.save(any(Comment.class))).willReturn(comment);
        given(commentMapper.toCommentResponse(comment)).willReturn(responseFor(comment));
        ...
        then(auditPublisher)
                ...
                .publishAudit(
                        ...
                        eq(comment.getId()),
                        eq(agentUser),
                        eq(AuditLog.AuditAction.COMMENT_EDITED),
                        any(),
                        isNull(),
                        any());
    }
    ...
    void updateComment_policyDenied_throwsAccessDenied() {
        ...
        willThrow(
                        new AccessDeniedException(
                                "Comments can only be edited/deleted within 15 minutes"))
                .given(accessPolicy)
                .assertCanMutate(endUser, comment);
    ...
    void deleteComment_valid_publishesAuditAndDeletes() {
        ...
        then(auditPublisher)
                ...
                .publishAudit(
                        ...
                        eq(comment.getId()),
                        eq(agentUser),
                        eq(AuditLog.AuditAction.COMMENT_DELETED),
                        any(),
                        isNull(),
                        any());
    ...
    void deleteComment_policyDenied_throwsAccessDenied() {
        ...
        given(commentQuery.findOrThrow(200L, endUser)).willReturn(oldComment);
        willThrow(
                        new AccessDeniedException(
                                "Comments can only be edited/deleted within 15 minutes"))
                .given(accessPolicy)
                .assertCanMutate(endUser, oldComment);

        assertThatThrownBy(() -> commentService.deleteComment(200L, endUser))
                .isInstanceOf(AccessDeniedException.class)
                .hasMessageContaining("15 minutes");
    ...
    private CommentResponse responseFor(final Comment c) {
