...
class TicketServiceImplTest {
    ...
    void setUp() {
        ...
        endUser = User.builder().id(2L).name("John Public").email("john@citizen.za").role(User.Role.USER).active(true)
                .build();
        agent = Agent.builder().id(1L).user(agentUser).availability(Agent.Availability.ONLINE).build();
        openTicket = Ticket.builder().id(100L).subject("Login broken").description("Cannot access dashboard")
                .status(Ticket.Status.OPEN).priority(Ticket.Priority.HIGH).requester(endUser)
    ...
    void createTicket_validRequest_savesInitializesSlaAndDispatchesEvent() {
        ...
        then(eventDispatcher).should(times(1)).publish(eq(openTicket), eq(endUser),
                eq(AuditLog.AuditAction.TICKET_CREATED), eq(null), eq(Ticket.Status.OPEN.name()), any(), eq(null));
    }
    ...
    void createTicket_noPriority_defaultsMedium() {
        ...
        Ticket mediumTicket = Ticket.builder().id(101L).subject("Test").description("Test desc")
                .status(Ticket.Status.OPEN).priority(Ticket.Priority.MEDIUM).requester(endUser).build();
    ...
    void createTicket_withAssigneeId_loadsAgentAndPublishesAssignedEvent() {
        ...
        Ticket ticketWithAssignee = Ticket.builder().id(102L).subject("Assigned ticket").description("desc")
                .status(Ticket.Status.OPEN).priority(Ticket.Priority.MEDIUM).requester(endUser).assignee(agent).build();
    ...
    void createTicket_unknownAssignee_throwsNotFound() {
        ...
        assertThatThrownBy(() -> ticketService.createTicket(req, endUser)).isInstanceOf(ResourceNotFoundException.class)
    ...
    void updateTicket_openToInProgress_recordsFirstResponse() {
        ...
        then(eventDispatcher).should(times(1)).publish(eq(openTicket), eq(agentUser),
                eq(AuditLog.AuditAction.STATUS_CHANGED), eq("OPEN"), eq("IN_PROGRESS"), eq(null), eq(null));
    }
    ...
    void updateTicket_inProgressToResolved_recordsResolution() {
        Ticket inProgress = Ticket.builder().id(100L).subject("Login broken").description("desc")
                .status(Ticket.Status.IN_PROGRESS).priority(Ticket.Priority.HIGH).requester(endUser).assignee(agent)
                .build();
        UpdateTicketRequest req = new UpdateTicketRequest();
    ...
    void updateTicket_resolvedToClosed_dispatchesTicketClosed() {
        Ticket resolved = Ticket.builder().id(100L).subject("Login broken").description("desc")
                .status(Ticket.Status.RESOLVED).priority(Ticket.Priority.HIGH).requester(endUser).assignee(agent)
                .build();
        UpdateTicketRequest req = new UpdateTicketRequest();
        ...
        then(eventDispatcher).should(times(1)).publish(eq(resolved), eq(agentUser),
                eq(AuditLog.AuditAction.TICKET_CLOSED), eq("RESOLVED"), eq("CLOSED"), eq(null), eq(null));
    }
    ...
    void updateTicket_invalidTransition_throwsAndNeverSaves() {
        Ticket closed = Ticket.builder().id(200L).subject("Old issue").description("Done").status(Ticket.Status.CLOSED)
                .requester(endUser).createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();
        UpdateTicketRequest req = new UpdateTicketRequest();
        ...
        assertThatThrownBy(() -> ticketService.updateTicket(200L, req, agentUser))
                .isInstanceOf(InvalidStatusTransitionException.class).hasMessageContaining("CLOSED")
    ...
    void updateTicket_assignAgent_dispatchesAssignedEvent() {
        ...
        then(eventDispatcher).should(times(1)).publish(eq(openTicket), eq(agentUser),
                eq(AuditLog.AuditAction.ASSIGNED_TO_AGENT), eq("Unassigned"), eq(agentUser.getName()), eq(null),
                eq(null));
    }
    ...
    void updateTicket_sameAssignee_noEventDispatched() {
        Ticket assigned = Ticket.builder().id(100L).subject("Login broken").description("desc")
                .status(Ticket.Status.OPEN).priority(Ticket.Priority.HIGH).requester(endUser).assignee(agent).build();
        UpdateTicketRequest req = new UpdateTicketRequest();
        req.setAssigneeId(1L);
    ...
    void updateTicket_priorityChange_dispatchesPriorityChanged() {
        ...
        then(eventDispatcher).should(times(1)).publish(eq(openTicket), eq(agentUser),
                eq(AuditLog.AuditAction.PRIORITY_CHANGED), eq("HIGH"), eq("URGENT"), eq(null), eq(null));
    }
    ...
    void updateTicket_escalationFromInProgress_marksEscalatedStatus() {
        ...
        then(eventDispatcher).should(times(1)).publish(eq(inProgress), eq(agentUser),
                eq(AuditLog.AuditAction.ESCALATED), eq("false"), eq("true"), eq("SLA risk"), eq("SLA risk"));
    }
    ...
    void updateTicket_alreadyEscalated_noEventDispatched() {
        Ticket alreadyEscalated = Ticket.builder().id(100L).subject("Login broken").description("desc")
                .status(Ticket.Status.ESCALATED).priority(Ticket.Priority.HIGH).requester(endUser).escalated(true)
                .build();
        UpdateTicketRequest req = new UpdateTicketRequest();
        req.setEscalated(true);
    ...
    void deleteTicket_valid_dispatchesEventAndDeletes() {
        ...
        then(eventDispatcher).should(times(1)).publish(eq(openTicket), eq(agentUser),
                eq(AuditLog.AuditAction.TICKET_DELETED), eq("OPEN"), eq("DELETED"), any(), eq(null));
    ...
    private TicketResponse responseFor(Ticket ticket) {
        return TicketResponse.builder().id(ticket.getId()).subject(ticket.getSubject())
                .description(ticket.getDescription()).status(ticket.getStatus()).priority(ticket.getPriority())
                .category(ticket.getCategory()).escalated(ticket.isEscalated()).createdAt(ticket.getCreatedAt())
