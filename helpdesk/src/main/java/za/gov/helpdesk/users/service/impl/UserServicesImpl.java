...
public class UserServicesImpl implements UserService {
    ...
    public UserResponse createUser(CreateUserRequest request, User actor) {
        ...
        if (userRepository.existsByEmail(request.getEmail())) {
            throw new DuplicateResourceException("A user with '" + request.getEmail() + "' already exists");
        }

        User user = User.builder().name(request.getName()).email(request.getEmail())
                .passwordHash(passwordEncoder.encode(request.getPassword()))
                .role(request.getRole() != null ? request.getRole() : User.Role.USER).phone(request.getPhone())
                .timezone(request.getTimezone() != null ? request.getTimezone() : "Africa/Johannesburg").active(true)
                .build();

        User savedUser = userRepository.save(user);

        auditPublisher.publishAudit(AuditLog.EntityType.USER, savedUser.getId(), actor,
                AuditLog.AuditAction.USER_CREATED, null, savedUser.getEmail(),
    ...
    public UserResponse updateUser(Long id, UpdateUserRequest request, User actor) {
        ...
        if (request.getName() != null && !request.getName().equals(user.getName())) {
            changes.append("name: ").append(user.getName()).append(" -> ").append(request.getName());
        ...
        if (!changes.isEmpty()) {
            auditPublisher.publishAudit(AuditLog.EntityType.USER, savedUser.getId(), actor,
                    AuditLog.AuditAction.USER_UPDATED, null, null, changes.toString().trim());
        }
