...
public class TicketEmailNotificationConsumer {
    ...
    public void handle(TicketEmailNotificationMessage message, Message rawMessage, Channel channel) throws IOException {

        long tag = rawMessage.getMessageProperties().getDeliveryTag();

        log.info("Ticket email notification received: trigger={} ticket={}", message.getTrigger(),
        ...
        try {
            ...
            log.info("Ticket email ACKed: trigger={} ticket={}", message.getTrigger(), message.getTicketNumber());

        } catch (Exception e) {
            // SMTP failed - requeue so it retries
            // After max-attempts it goes to TICKET_EMAIL_DLQ automatically
            log.warn("Ticket email failed, re-queuing: trigger={} ticket={} error={}", message.getTrigger(),
                    message.getTicketNumber(), e.getMessage());
