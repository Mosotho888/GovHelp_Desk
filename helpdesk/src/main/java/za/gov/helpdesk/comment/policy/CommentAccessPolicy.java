...
public class CommentAccessPolicy {
    ...
    public boolean canMutate(User user, Comment comment) {
        ...
        boolean withinWindow = comment.getCreatedAt().isAfter(LocalDateTime.now().minusMinutes(EDIT_WINDOW_MINUTES));
    ...
    public void assertCanMutate(User user, Comment comment) {
        if (!canMutate(user, comment)) {
            throw new AccessDeniedException("Comments can only be edited/deleted within " + EDIT_WINDOW_MINUTES
