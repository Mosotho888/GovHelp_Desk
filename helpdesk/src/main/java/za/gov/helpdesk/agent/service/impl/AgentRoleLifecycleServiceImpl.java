...
public class AgentRoleLifecycleServiceImpl implements AgentRoleLifecycleService {
    ...
    @Override
    @Transactional
    public void handleRoleChange(User target, User.Role oldRole, User.Role newRole, User admin) {
    ...
    private void handlePromotion(User target, User.Role oldRole, User admin) {
        if (agentRepository.existsByUserId(target.getId())) {

            agentRepository.findByUserId(target.getId()).ifPresent(agent -> {
    ...
    private void handleDemotion(User target, User admin) {
        agentRepository.findByUserId(target.getId()).ifPresent(agent -> {
            ...
            Agent.Availability previousAvailability = agent.getAvailability();
