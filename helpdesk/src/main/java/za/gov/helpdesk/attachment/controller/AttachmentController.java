...
public class AttachmentController {
    ...
    @PostMapping(value = "/v1/tickets/{ticketId}/attachments", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "Upload files to a ticket (max 5 files, 20 MB each)")
    public ResponseEntity<List<AttachmentResponse>> uploadAttachments(@PathVariable Long ticketId,
            @RequestParam("file") List<MultipartFile> files, @AuthenticationPrincipal CustomUserDetails principal) {
    ...
    public ResponseEntity<Resource> downloadAttachment(@PathVariable Long attachmentId,
        ...
        return ResponseEntity.ok()
                ...
                .contentType(MediaType.parseMediaType(attachment.getContentType())).body(resource);
    }
