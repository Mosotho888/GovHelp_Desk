...
public class OutboxMetrics {
    ...
    public OutboxMetrics(final MeterRegistry registry) {
        ...
        this.published =
                Counter.builder("helpdesk.outbox.published")
                        .description("Outbox events successfully published to RabbitMQ")
                        .register(registry);

        this.failed =
                Counter.builder("helpdesk.outbox.failed")
                        .description(
                                "Outbox publish attempts that resulted in an exception (will"
                                        + " retry)")
                        .register(registry);

        this.deadLetter =
                Counter.builder("helpdesk.outbox.dead.letter")
                        .description(
                                "Outbox events that exhausted all retries and are permanently"
                                        + " stuck")
                        .register(registry);

        Gauge.builder("helpdesk.outbox.pending.events", pendingGauge, AtomicLong::get)
                .description("Current number of PENDING events waiting in the outbox table")
                .register(registry);
    }

    public void setPendingGauge(final long pending) {
