...
public class OutboxMetrics {
    ...
    public OutboxMetrics(MeterRegistry registry) {
        ...
        this.failed = Counter.builder("helpdesk.outbox.failed")
                .description("Outbox publish attempts that resulted in an exception (will retry)")
                .register(registry);

        this.deadLetter = Counter.builder("helpdesk.outbox.dead.letter")
                .description("Outbox events that exhausted all retries and are permanently stuck")
                .register(registry);
