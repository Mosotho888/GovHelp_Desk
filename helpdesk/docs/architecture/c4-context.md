...
This diagram shows GovHelpDesk as a single black-box system, its users, and the external systems it depends on. See [
`docs/architecture/c4-container.md`](c4-container.md) for the next level of detail.

```mermaid
graph TB
    employee["Employees<br/>(Person)<br/>Submits and tracks support tickets"]
    agent["Support Agent<br/>(Person)<br/>Works assigned tickets, replies, resolves"]
    admin["Administrator<br/>(Person)<br/>Manages users, agents, and oversight"]
    ...
    employee -->|" HTTPS / REST, JWT auth "| system
    ...
    employee -.->|" DNS resolution + TLS "| cloudflare
...
|---------------|---------|-----------------------------------------------------------------------------------------------------------------------|
| Employees     | `USER`  | Creates tickets, comments on their own tickets, uploads attachments, tracks status                                    |
...
| Administrator | `ADMIN` | Full oversight: manages users and agents, deletes tickets, views audit logs, unlocks accounts, resets passwords       |
